...
function Collaboration() {
    ...
    const [teamMembers, setTeamMembers] = useState([
        { id: 101, name: 'Sarah Johnson', avatar: 'https://readdy.ai/api/search-image?query=professional%20headshot%20of%20a%20young%20woman%20with%20brown%20hair%20and%20friendly%20smile%2C%20business%20attire%2C%20neutral%20background%2C%20high%20quality%20portrait%20photo%2C%20soft%20lighting&width=40&height=40&seq=user1sm&orientation=squarish', isOnline: true, lastSeen: 'Just now', role: 'Admin' },
        { id: 102, name: 'Michael Chen', avatar: 'https://readdy.ai/api/search-image?query=professional%20headshot%20of%20an%20asian%20man%20with%20glasses%20wearing%20business%20casual%20attire%2C%20neutral%20background%2C%20high%20quality%20portrait%20photo%2C%20soft%20lighting&width=40&height=40&seq=user2sm&orientation=squarish', isOnline: true, lastSeen: 'Just now', role: 'Manager' },
        { id: 103, name: 'Alex Rodriguez', avatar: 'https://readdy.ai/api/search-image?query=professional%20headshot%20of%20a%20latino%20man%20with%20short%20dark%20hair%20wearing%20a%20blue%20shirt%2C%20neutral%20background%2C%20high%20quality%20portrait%20photo%2C%20soft%20lighting&width=40&height=40&seq=user3sm&orientation=squarish', isOnline: false, lastSeen: '2 hours ago', role: 'Member' },
        { id: 104, name: 'Emma Wilson', avatar: 'https://readdy.ai/api/search-image?query=professional%20headshot%20of%20a%20blonde%20woman%20with%20medium%20length%20hair%20wearing%20professional%20attire%2C%20neutral%20background%2C%20high%20quality%20portrait%20photo%2C%20soft%20lighting&width=40&height=40&seq=user4sm&orientation=squarish', isOnline: true, lastSeen: 'Just now', role: 'Member' },
        { id: 105, name: 'David Thompson', avatar: 'https://readdy.ai/api/search-image?query=professional%20headshot%20of%20a%20middle%20aged%20man%20with%20short%20brown%20hair%20wearing%20a%20suit%2C%20neutral%20background%2C%20high%20quality%20portrait%20photo%2C%20soft%20lighting&width=40&height=40&seq=user5sm&orientation=squarish', isOnline: false, lastSeen: '5 hours ago', role: 'Member' }
    ]);
    ...
    const handleSendMessage = () => {
        ...
        if (newMessage.includes('@')) {
            ...
            mentions.forEach(mention => {
                ...
                const user = teamMembers.find(m => m.name.toLowerCase().includes(username.toLowerCase()));
    ...
    const handleMentionSelect = (user) => {
        ...
        const messageAfter = newMessage.substring(mentionPosition + mentionQuery.length + 1);
    ...
    const handleCreateGroup = () => {
        ...
        alert(`Group "${newGroupName}" created with ${selectedMembers.length + 1} members!`);
    };
    ...
    const filteredMessages = messages.filter(msg => {
        ...
        if (activeFilter === 'Mentions') return msg.mentionedUsers && msg.mentionedUsers.includes(currentUser.id);
        return true;
    });
            ...
            <div className="flex-grow-1 d-flex overflow-hidden">
                ...
                <div className="d-none d-lg-block col-lg-3 border-end bg-card p-3 overflow-auto">
                    ...
                    <div className="btn-group w-100 mb-3">
                        <button 
                            className={`btn btn-sm ${activeChat === 'group' ? 'btn-primary' : 'btn-outline-secondary'}`}
                            onClick={() => setActiveChat('group')}
                        >
                            Groups
                        </button>
                        <button 
                            className={`btn btn-sm ${activeChat === 'private' ? 'btn-primary' : 'btn-outline-secondary'}`}
                        ...
                        <>
                            ...
                            <ul className="list-unstyled">
                                {teamMembers.map(member => (
                                    <li 
                                        key={member.id} 
                                        className={`mb-2 p-2 rounded ${activePrivateChat === member.id ? 'bg-primary text-white' : ''}`}
                                            ...
                                            <div className="position-relative me-2">
                                                ...
                                                {member.isOnline ? (
                                                    <span className="position-absolute bottom-0 end-0 bg-success rounded-circle" 
                                                        style={{ width: '10px', height: '10px', border: '2px solid #f8f9fa' }}>
                                                    </span>
                                                ) : null}
                                            </div>
                                            <div>
                                                <div>{member.name}</div>
                                                <small className={member.isOnline ? 'text-success' : 'text-muted'}>
                    ...
                    <ul className="list-unstyled">
                        {teamMembers.map(member => (
                            <li key={member.id} className="mb-2 d-flex align-items-center">
                                ...
                                {member.role === 'Manager' && <span className="badge bg-warning ms-2">Manager</span>}
                            </li>
                        ))}
                    </ul>
                </div>
                ...
                <div className="col-12 col-lg-9 d-flex flex-column">
                    ...
                    <div className="p-3 border-bottom bg-main d-flex justify-content-between align-items-center">
                            ...
                            <small className="text-white">
                                {activePrivateChat 
                                    ? teamMembers.find(m => m.id === activePrivateChat)?.isOnline 
                                        ? 'Online' 
                                        : `Last seen ${teamMembers.find(m => m.id === activePrivateChat)?.lastSeen}`
                                    : `${teamMembers.length + 1} members • Last activity 11:32 AM`}
                            </small>
                        </div>
                        <div className="d-flex">
                            ...
                            <div className="dropdown">
                                <button className="btn btn-sm btn-outline-light dropdown-toggle" 
                                    data-bs-toggle="dropdown">
                                    <i className="fas fa-ellipsis-v"></i>
                                </button>
                                <ul className="dropdown-menu dropdown-menu-end">
                                    <li><button className="dropdown-item">View Members</button></li>
                                    {activeChat === 'group' && (
                                        <li><button className="dropdown-item">Group Settings</button></li>
                                    )}
                                    <li><button className="dropdown-item">Clear History</button></li>
                                    <li><hr className="dropdown-divider" /></li>
                                    <li><button className="dropdown-item text-danger">Leave Chat</button></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                    ...
                    {/* Messages Area */}
                    <div className="flex-grow-1 p-3 bg-main overflow-auto" style={{ backgroundColor: '#1e1e1e' }}>
                        {filteredMessages.map((message) => (
                            <div 
                                key={message.id} 
                                className={`mb-3 ${message.senderId === currentUser.id ? 'text-end' : ''}`}
                            >
                                {message.replyTo && (
                                    <div className={`small text-muted mb-1 ${message.senderId === currentUser.id ? 'text-end' : ''}`}>
                                        Replying to: {messages.find(m => m.id === message.replyTo)?.content.substring(0, 50)}...
                                    </div>
                                )}
                                <div className={`d-flex ${message.senderId === currentUser.id ? 'justify-content-end' : ''}`}>
                                    ...
                                    <div 
                                        className={`rounded p-3 position-relative ${message.senderId === currentUser.id ? 'bg-primary' : 'bg-card'}`}
                                        style={{ maxWidth: '75%' }}
                                    >
                                        <div className="d-flex justify-content-between align-items-center mb-1">
                                            <strong className={message.senderId === currentUser.id ? 'text-white' : 'text-white'}>
                                                {message.sender}
                                            </strong>
                                            <small className={message.senderId === currentUser.id ? 'text-white-50' : 'text-white-50'}>
                                                {message.timestamp}
                                                {message.isEdited && <span className="ms-1">(edited)</span>}
                                            </small>
                                        </div>
                                        ...
                                        {/* Message actions */}
                                        <div className={`position-absolute ${message.senderId === currentUser.id ? 'left-0 start-100' : 'right-0 end-100'} px-2 d-flex`}>
                                            <button 
                                        ...
                                        {message.reactions.length > 0 && (
                                            <div className="d-flex flex-wrap gap-1 mt-2">
                                                {message.reactions.map((reaction, idx) => (
                                                    <button 
                                                        key={idx} 
                                                        className={`btn btn-sm p-0 px-1 rounded-pill ${reaction.reacted ? 'bg-white' : message.senderId === currentUser.id ? 'bg-white-10' : 'bg-light'}`}
                                        ...
                                        {/* Seen status */}
                                        {activePrivateChat && message.senderId === currentUser.id && (
                                            <div className="text-end mt-1">
                                                <small className="text-white-50">
                                                    {message.seenBy.includes(activePrivateChat) ? 'Seen' : 'Delivered'}
                                                </small>
                                            </div>
                                        )}
                                    </div>
                            ...
                            <div className="d-flex align-items-center mb-3">
                                ...
                                <div className="rounded p-2 bg-card">
                                    <div className="d-flex align-items-center">
                                        ...
                                        <small className="ms-2 text-white">{typingUser.name} is typing...</small>
                                    </div>
                                </div>
                            </div>
                        )}
                        ...
                        <div className="position-relative">
                            ...
                            <div className="d-flex justify-content-between align-items-center">
                                ...
                                <button
                                    ...
                                    disabled={editingMessageId ? editedMessageContent.trim() === '' : newMessage.trim() === ''}
                                    className={`btn ${editingMessageId ? 'btn-warning' : newMessage.trim() === '' ? 'btn-outline-primary' : 'btn-primary'}`}
                                >
                                    {editingMessageId ? 'Save Edit' : 'Send'} <i className="fas fa-paper-plane ms-1"></i>
                                </button>
                            </div>
