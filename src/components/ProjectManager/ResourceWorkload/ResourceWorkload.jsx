...
function ResourceWorkload() {
  ...
  const taskData = [
    { id: 'TSK-001', name: 'Database Migration', resource: 'Computing', consumption: 45, priority: 'High', startTime: '08:00 AM', duration: '4h', progress: 75, status: 'Running' },
    { id: 'TSK-002', name: 'Server Maintenance', resource: 'Network', consumption: 30, priority: 'Medium', startTime: '09:30 AM', duration: '2h', progress: 50, status: 'Running' },
    { id: 'TSK-003', name: 'Backup Process', resource: 'Storage', consumption: 80, priority: 'High', startTime: '10:00 AM', duration: '3h', progress: 25, status: 'Running' },
    { id: 'TSK-004', name: 'Code Deployment', resource: 'Computing', consumption: 60, priority: 'Medium', startTime: '11:00 AM', duration: '1h', progress: 90, status: 'Running' },
    { id: 'TSK-005', name: 'Security Audit', resource: 'Human', consumption: 50, priority: 'Low', startTime: '01:00 PM', duration: '5h', progress: 10, status: 'Running' },
    { id: 'TSK-006', name: 'Data Analysis', resource: 'Computing', consumption: 70, priority: 'Medium', startTime: '02:30 PM', duration: '4h', progress: 0, status: 'Pending' },
    { id: 'TSK-007', name: 'Network Optimization', resource: 'Network', consumption: 40, priority: 'Low', startTime: '09:00 AM', duration: '2h', progress: 100, status: 'Completed' },
  ];
          ...
          <div className="d-flex justify-content-between align-items-center py-3">
            ...
            <div className="d-flex align-items-center">
              ...
              <div className="dropdown">
                ...
                >
                  {resourceView === 'all' ? 'All Resources' : resourceView.charAt(0).toUpperCase() + resourceView.slice(1)}
                </button>
                <ul className="dropdown-menu" aria-labelledby="resourceFilterDropdown">
                  <li><button className="dropdown-item" onClick={() => setResourceView('all')}>All Resources</button></li>
                  <li><button className="dropdown-item" onClick={() => setResourceView('computing')}>Computing</button></li>
                  <li><button className="dropdown-item" onClick={() => setResourceView('storage')}>Storage</button></li>
                  <li><button className="dropdown-item" onClick={() => setResourceView('network')}>Network</button></li>
                  <li><button className="dropdown-item" onClick={() => setResourceView('human')}>Human</button></li>
                </ul>
              </div>
            </div>
          </div>
              ...
              <div className="timeline-container" style={{ minWidth: '800px' }}>
                ...
                {resourceTypes.map((resource, index) => (
                  <div key={index} className="d-flex py-2 border-bottom">
                    ...
                    <div className="flex-grow-1 position-relative" style={{ height: '40px' }}>
                      {/* Capacity threshold line */}
                      <div className="position-absolute top-0 start-0 end-0 border-top border-danger border-dashed" style={{ top: '75%' }}></div>
                      ...
                      {taskData
                        ...
                        .map((task, taskIndex) => {
                          ...
                          const startMinute = parseInt(task.startTime.split(':')[0].includes('PM') && startHour !== 12 ? 
                            startHour + 12 : startHour);
                          const durationHours = parseFloat(task.duration.replace('h', ''));
                          ...
                          return (
                            <div 
                              key={taskIndex}
                              className={`position-absolute rounded ${getStatusColor(task.status)} opacity-75`}
                              style={{ 
                                left: `${startPosition}%`, 
                                width: `${width}%`,
                                height: '20px',
