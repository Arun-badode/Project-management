...
const ShiftAllocation = () => {
  ...
  // Updated getShiftLabel function to fix icon display
  const getShiftLabel = (start, end, type, otherType, shiftSymbol) => {
    ...
    // Handle special cases for otherType first
    ...
    // If a direct shiftSymbol is provided, use it
    if (shiftSymbol && shiftLabels[shiftSymbol]) {
    ...
    // Match by exact time range (Most reliable for your data)
    if (start && end) {
      const formattedTime = `${start} - ${end}`;
      const label = shiftLabels[formattedTime];
      if (label) {
        return (
          <span
            style={{
    ...
    // Fallback: Try to map common shiftType values to symbols based on convention
    // This is a guess and might need adjustment based on your business logic
    let mappedSymbol = "";
    if (type === "Night Shift" || type === "Night") {
      mappedSymbol = "δ"; // Assuming night shift is δ
    } else if (type === "Morning Shift") {
      mappedSymbol = "α"; // Assuming morning shift is α
    } else if (type === "Full Day") {
      mappedSymbol = "α"; // You can adjust this mapping
    } else if (type === "Half Day") {
      mappedSymbol = "β"; // You can adjust this mapping
    }

    if (mappedSymbol && shiftLabels[mappedSymbol]) {
      const label = shiftLabels[mappedSymbol];
      return (
        <span
          style={{
            backgroundColor: label.bg,
    ...
    // Final fallback: Show the shift type if no symbol matches
    if (type && !shiftLabels[type]) {
      return (
        <span
          style={{
  ...
  const fetchUserRole = async () => {
    try {
      const token = localStorage.getItem("authToken");
      const response = await axios.get(`${BASE_URL}auth/getUserProfile`, {
        headers: {
          Authorization: `Bearer ${token}`,
        }
      });
  ...
  const isTeamMember = localStorage.getItem("userRole") == "Team-Member";
          ...
          <div className="container">
            <div className="d-flex justify-content-between align-items-center">
              ...
              <div className="d-flex gap-3">
                {/* Only show Add New Shift button if user is not a team member and has permission */}
                {!isTeamMember && CreateShiftPermission === 1 &&
                  <button
                        ...
                        <td key={dayIndex} className="position-relative align-top" style={{ minHeight: "80px" }}>
                          {date.getDay() === 0 ? ( // Check if the date is actually Sunday (getDay() === 0)
                            ...
                            <>
                              ...
                              {/* Add/Remove buttons - only show if user is not a team member */}
                              {!isTeamMember && (
