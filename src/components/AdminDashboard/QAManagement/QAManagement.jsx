...
const QAManagement = () => {
    ...
    const [tasks, setTasks] = useState([
        { id: 'QA-001', title: 'Login Page Testing', module: 'Authentication', priority: 'High', createdBy: 'John Doe', createdAt: '2024-06-15', status: 'Open', assignedTo: null, lastUpdated: '2024-06-15', resultFile: null },
        { id: 'QA-002', title: 'Payment Gateway Integration', module: 'Payment', priority: 'Critical', createdBy: 'Jane Smith', createdAt: '2024-06-14', status: 'In Progress', assignedTo: 'Alice Johnson', lastUpdated: '2024-06-16', resultFile: null },
        { id: 'QA-003', title: 'Dashboard UI Testing', module: 'UI/UX', priority: 'Medium', createdBy: 'Mike Wilson', createdAt: '2024-06-13', status: 'Completed', assignedTo: 'Bob Brown', lastUpdated: '2024-06-17', resultFile: 'test-results.pdf' },
        { id: 'QA-004', title: 'API Endpoint Validation', module: 'Backend', priority: 'High', createdBy: 'Sarah Davis', createdAt: '2024-06-12', status: 'Failed', assignedTo: 'Charlie Wilson', lastUpdated: '2024-06-16', resultFile: null },
        { id: 'QA-005', title: 'Mobile Responsiveness', module: 'UI/UX', priority: 'Low', createdBy: 'Tom Anderson', createdAt: '2024-06-11', status: 'Open', assignedTo: null, lastUpdated: '2024-06-11', resultFile: null }
    ]);
    ...
    const filteredTasks = tasks.filter(task => {
        return (
            (filters.search === '' || task.title.toLowerCase().includes(filters.search.toLowerCase()) || task.module.toLowerCase().includes(filters.search.toLowerCase())) &&
    ...
    const selfAssignTask = (taskId) => {
        setTasks(tasks.map(task =>
            task.id === taskId
                ? { ...task, assignedTo: currentUser, status: 'In Progress', lastUpdated: new Date().toISOString().split('T')[0] }
                : task
        ));
    ...
    const handleFileUpload = () => {
        ...
        const fileData = {
            ...
            serverPath: `/qa/uploads/${Date.now()}-${uploadForm.file?.name || 'test-file.pdf'}`,
    ...
    return (
        <div className="p-4 py-4">
            {/* Toast Notification */}
            {showToast.show && (
                <div className={`toast position-fixed top-0 end-0 m-3 show`} style={{ zIndex: 1050 }}>
                    <div className={`toast-header bg-${showToast.type === 'error' ? 'danger' : 'success'} text-white`}>
                        <strong className="me-auto">Notification</strong>
                        <button type="button" className="btn-close btn-close-white" onClick={() => setShowToast({ show: false, message: '', type: '' })}></button>
                    </div>
            ...
            <ul className="nav nav-tabs mb-4">
                <li className="nav-item">
                    <button className={`nav-link ${activeTab === 'taskPool' ? 'active' : ''}`} onClick={() => setActiveTab('taskPool')}>
                        📋 QA Task Pool
                    </button>
                </li>
                <li className="nav-item">
                    <button className={`nav-link ${activeTab === 'selfAssign' ? 'active' : ''}`} onClick={() => setActiveTab('selfAssign')}>
                        👤 Self-Assign Tasks
                    </button>
                </li>
                <li className="nav-item">
                    <button className={`nav-link ${activeTab === 'completion' ? 'active' : ''}`} onClick={() => setActiveTab('completion')}>
                        ✅ Completion Status
                    </button>
                </li>
                <li className="nav-item">
                                        ...
                                        <tbody className='text-white'>
                                            {filteredTasks.map(task => (
                                                <tr key={task.id}>
                                                    ...
                                                    <td>
                                                        <button className="btn btn-sm btn-outline-primary me-1" onClick={() => setShowModal({ show: true, type: 'viewDetails', data: task })}>
                                                            👁️ View
                                                        </button>
                                                        {task.status === 'Open' && (
                                                            <button className="btn btn-sm btn-success" onClick={() => setShowModal({ show: true, type: 'selfAssign', data: task })}>
                                                                ✋ Self-Assign
                                                            </button>
                                                        )}
                                                    </td>
                                    ...
                                    <div key={task.id} className="card mb-3">
                                        <div className="card-body">
                                            ...
                                            <div className="d-flex gap-2 flex-wrap">
                                                <button className="btn btn-sm btn-outline-primary" onClick={() => setShowModal({ show: true, type: 'viewDetails', data: task })}>
                                                    👁️ View
                                                </button>
                                                {task.status === 'Open' && (
                                                    <button className="btn btn-sm btn-success" onClick={() => setShowModal({ show: true, type: 'selfAssign', data: task })}>
                    ...
                    <div className="card bg-card ">
                        ...
                        <div className="card-body">
                            ...
                            <div className="row mb-3">
                                ...
                                <div className="col-md-3 mb-2">
                                    <select className="form-select" value={filters.module} onChange={(e) => setFilters({ ...filters, module: e.target.value })}>
                                        ...
                                        <tbody>
                                            {filteredTasks.filter(task => task.assignedTo).map(task => (
                                                <tr key={task.id}>
                                                    ...
                                                    <td>
                                                        {task.resultFile ? (
                                                            <a href="#" className="text-success">📄 {task.resultFile}</a>
                                                        ) : (
                                ...
                                {filteredTasks.filter(task => task.assignedTo).map(task => (
                                    <div key={task.id} className="card mb-3">
                                        <div className="card-body">
                                            ...
                                            {task.resultFile && (
                                                <p className="card-text small">
                                                    <strong>Result File:</strong> <a href="#" className="text-success">📄 {task.resultFile}</a>
                                                </p>
                                            )}
                                            <div className="d-flex gap-2 flex-wrap">
                                                <button className="btn btn-sm btn-outline-primary" onClick={() => setShowModal({ show: true, type: 'updateStatus', data: task })}>
                                                    ✏️ Update
                                                </button>
                                                <button className="btn btn-sm btn-outline-info" onClick={() => setShowModal({ show: true, type: 'viewDetails', data: task })}>
                                                    👁️ View
                                ...
                                <div className="card-body ">
                                    <div>
                                        <div className="mb-3">
                                            ...
                                            <select className="form-select" value={uploadForm.taskId} onChange={(e) => setUploadForm({ ...uploadForm, taskId: e.target.value })}>
                                        ...
                                        <div className="mb-3">
                                            ...
                                            <input type="file" className="form-control" onChange={(e) => setUploadForm({ ...uploadForm, file: e.target.files[0] })} accept=".pdf,.doc,.docx,.xlsx,.png,.jpg" />
                                        </div>
                                        <div className="mb-3">
                                            <label className="form-label">File Type</label>
                                            <select className="form-select" value={uploadForm.fileType} onChange={(e) => setUploadForm({ ...uploadForm, fileType: e.target.value })}>
                                        ...
                                        <div className="mb-3">
                                            ...
                                            <textarea className="form-control" value={uploadForm.comments} onChange={(e) => setUploadForm({ ...uploadForm, comments: e.target.value })} rows="3" placeholder="Add any additional comments..."></textarea>
                                        </div>
                                        <button type="button" className="btn btn-primary w-100" onClick={handleFileUpload}>
                                            📤 Upload File
                                        </button>
                                    </div>
                                </div>
                                            ...
                                            {uploadedFiles.map((file, index) => (
                                                <div key={index} className="list-group-item bg-card px-0">
                                                    ...
                                                    <div className="mt-2 d-flex flex-wrap gap-1">
                                                        ...
                                                        <button className="btn btn-sm btn-outline-info">👁️ Preview</button>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                                    ...
                                    <div className="modal-body bg-card">
                                        <div className="row">
                                            <div className="col-md-6">
                                                ...
                                                <p><strong>Module:</strong> {showModal.data.module}</p>
                                                <p><strong>Priority:</strong> <span className={`badge bg-${getPriorityColor(showModal.data.priority)}`}>{showModal.data.priority}</span></p>
                                                <p><strong>Status:</strong> <span className={`badge bg-${getStatusColor(showModal.data.status)}`}>{showModal.data.status}</span></p>
                                            </div>
                                            <div className="col-md-6">
                                                <p><strong>Created By:</strong> {showModal.data.createdBy}</p>
                                                <p><strong>Created At:</strong> {showModal.data.createdAt}</p>
                                                <p><strong>Assigned To:</strong> {showModal.data.assignedTo || 'Unassigned'}</p>
                                                <p><strong>Last Updated:</strong> {showModal.data.lastUpdated}</p>
                                            </div>
                                        </div>
                                        <div className="mt-3">
                            ...
                            {showModal.type === 'updateStatus' && (
                                <>
                                    ...
                                    <div className="modal-body bg-card">
                                        ...
                                        <div className="mt-3">
                                            <button className="btn btn-warning me-2 mb-2" onClick={() => updateTaskStatus(showModal.data.id, 'In Progress')}>🔄 In Progress</button>
                                            <button className="btn btn-success me-2 mb-2" onClick={() => updateTaskStatus(showModal.data.id, 'Completed')}>✅ Completed</button>
                                            <button className="btn btn-danger me-2 mb-2" onClick={() => updateTaskStatus(showModal.data.id, 'Failed')}>❌ Failed</button>
                                        </div>
                                    </div>
                                </>
                            )}
