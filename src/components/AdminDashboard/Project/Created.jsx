...
const Created = ({ data }) => {
  ...
  // Changed to object to track selected files per project
  ...
  const handleDeleteProject = (id) => {
    if (window.confirm("Are you sure to delete this project permanently?")) {
  ...
  const saveFileStatusUpdates = async (projectId) => {
    try {
      ...
      const projectSelectedFiles = selectedFiles[projectId] || [];

      // Update selected files with new status and deadline
      for (const fileId of projectSelectedFiles) {
        await axios.patch(
      ...
      // Clear selections for this project
  ...
  // Function to mark selected files as YTS
  const markSelectedFilesAsYTS = async (projectId) => {
    try {
      const projectSelectedFiles = selectedFiles[projectId] || [];
      
      if (projectSelectedFiles.length === 0) {
        alert("Please select at least one file to mark as YTS.");
        return;
      }
      ...
      alert("Selected files marked as YTS successfully!");
      ...
      console.error("Failed to mark files as YTS:", error);
      alert("Error marking files as YTS");
    }
  };

  // Function to toggle file selection
  const toggleFileSelection = (projectId, fileId) => {
  setSelectedFiles(prev => {
    const newState = { ...prev };
    if (!newState[projectId]) {
      newState[projectId] = [];
    }
    const projectFiles = [...newState[projectId]];
    const fileIndex = projectFiles.indexOf(fileId);
    if (fileIndex > -1) {
      projectFiles.splice(fileIndex, 1);
    } else {
      projectFiles.push(fileId);
    }
    newState[projectId] = projectFiles;
    return newState;
  });
};

  // Function to toggle all files selection for a project
  const toggleAllFilesSelection = (projectId, isChecked) => {
    const projectFiles = allFiles.filter(file => file.projectId === projectId);
    ...
    if (isChecked) {
      ...
      setSelectedFiles(prev => ({
        ...prev,
        [projectId]: projectFiles.map(file => file.id)
      }));
    } else {
      // Deselect all files for this project
      setSelectedFiles(prev => ({
        ...prev,
        [projectId]: []
      }));
  ...
  // Function to open calendar for a specific project
        ...
        <table className="table table-hover mb-0" style={{ minWidth: 900 }}>
          ...
          >
            <tr className="text-center">
              ...
              <th style={{ width: "6%" }}>Estimated Hrs</th>
              <th style={{ width: "6%" }}>Deadline</th>
              <th style={{ width: "6%" }}>Cost with Currency </th>
              <th style={{ width: "7%" }}>Cost in INR</th>
              <th style={{ width: "7%" }}>Files Count</th>
          ...
          <tbody>
            {projects
              ...
              .filter((project) => {
                // Filter projects where NOT all files have RFD status
                const projectFiles = allFiles.filter((file) => file.projectId === project.id);
                const allFilesRFD = projectFiles.length > 0 &&
                  projectFiles.every(file => file.status === "RFD");
              ...
              .map((project, index) => (
                <React.Fragment key={project.id}>
                  <tr className="text-center">
                    ...
                    <td style={{ maxWidth: "80ch", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {project.title}
                    ...
                    <td style={{ maxWidth: "16ch", wordWrap: "break-word" }}>{project.full_name}</td>
                    ...
                    <td style={{ maxWidth: "4ch", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {project.totalProjectPages ? Math.floor(project.totalProjectPages) : "-"}
                    </td>
                    <td style={{ maxWidth: "8ch", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {project.receiveDate
                        ? new Date(project.receiveDate).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: '2-digit' })
                        : "-"}
                    </td>
                    <td style={{ maxWidth: "6ch", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{project.estimatedHours || "-"}</td>
                    <td style={{ maxWidth: "10ch", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {fileDeadlines[project.id]
                    ...
                    <td style={{ maxWidth: "10ch", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {project.currency
                        ? `${project.currency || "USD"} ${parseFloat(project.totalCost || 0).toFixed(2)}`
                        : "-"}
                    </td>
                    <td style={{ maxWidth: "11ch", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        ...
                        <div className="mb-4">
                          <div className="d-flex justify-content-between align-items-center mb-3">
                            ...
                            <div className="d-flex gap-2">
                              <button
                                className="btn btn-sm btn-primary"
                                onClick={() => markSelectedFilesAsYTS(project.id)}
                                disabled={!selectedFiles[project.id] || selectedFiles[project.id].length === 0}
                              >
                                Mark Selected as YTS
                              </button>
                            </div>
                          </div>
      ...
      <th style={{ width: "8%" }}>
        <input
          ...
          checked={
            ...
            selectedFiles[project.id]?.length ===
              allFiles.filter((f) => f.projectId === project.id).length
          }
          onChange={(e) => toggleAllFilesSelection(project.id, e.target.checked)}
        />
      </th>
      ...
      .map((file) => (
        <tr key={file.id}>
          <td>
            <input
              type="checkbox"
              checked={selectedFiles[project.id]?.includes(file.id) || false}
              onChange={() => toggleFileSelection(project.id, file.id)}
            />
          </td>
          ...
          <td style={{ maxWidth: "10ch", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{file.languageName || "N/A"}</td>
          <td style={{ maxWidth: "10ch", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{file.applicationName || "N/A"}</td>
          <td>
            <select
          ...
          <td>
            {file.deadline
              ? new Date(file.deadline).toLocaleString('en-GB', {
                hour: '2-digit',
                minute: '2-digit',
                hour12: true,
                day: '2-digit',
              ...
              : "-"}
                          ...
                          <div className="d-flex justify-content-between align-items-center mt-3">
                            <div className="d-flex align-items-center gap-3">
                              <label className="form-label mb-0">Set Deadline for Selected Files:</label>
                              <div className="max-w-md mx-auto position-relative" ref={calendarRef}>
                                ...
                                {calendarOpen[project.id] && (
                                  <div className="calendar-dropdown position-absolute" style={{ zIndex: 1000, top: "100%", left: 0 }}>
                  ...
                  <div className="row mb-3">
                    <div className="col-md-6">
                      ...
                      <input
                        ...
                        name="projectTitle"  // Changed from title to match database
                    ...
                    <div className="col-md-6">
                      ...
                      <input
                        type="text"
                        className="form-control"
                        name="clientId"  // Changed from clientName to match database
