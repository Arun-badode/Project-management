...
const Project = () => {
 ...
 const {
  ...
  useEffect(() => {
    // You can determine if user is admin based on roleId or permissions
    // For example, if roleId is 1 or if user has specific admin permissions
    const adminRoleId = "1"; // Replace with your actual admin role ID
    setIsAdmin(roleId === adminRoleId);
  }, [roleId]);
  ...
  const exportToExcel = () => {
    ...
    if (activeTab === "created") {
      dataToExport = filteredProjects.map(project => ({
        ID: project.id,
        Title: project.title,
        ...
        Files: project.files.map(f => `${f.name} (${f.pageCount} pages)`).join(", "),
      ...
      dataToExport = filteredProjects.map(project => ({
        ...
        Cost: project.cost,
        INRCost: project.inrCost,
        ExpectedHours: project.performance?.expectedHours || "",
        ActualHours: project.performance?.actualHours || "",
        Stages: project.performance?.stages?.map(s => `${s.name} (${s.start} to ${s.end} by ${s.handler})`).join("; ") || ""
      }));
    }
    ...
    XLSX.utils.book_append_sheet(wb, ws, `${activeTab} Projects`);
    
    // Generate the Excel file and download it
    XLSX.writeFile(wb, `${activeTab}_projects_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
    ...
    <div className="conatiner-fluid bg-main mt-4">
      ...
      <div className="container-fluid pt-5 pb-4">
        ...
        {searchQuery && (
          <div className="alert alert-info mb-4">
            <div className="d-flex">
              ...
              <div className="flex-grow-1 d-flex justify-content-between align-items-center">
                <div>
                  Showing {filteredProjects.length} results for "{searchQuery}" in {activeTab} projects
                </div>
                <button
                  onClick={() => setSearchQuery("")}
                  className="btn btn-link p-0"
        ...
        {activeTab === "created" && (
          <div className="mb-4">
            <div className="d-flex justify-content-between align-items-center mb-3">
              <h2 className="h5 mb-0 text-light">Created Projects</h2>
              <button 
            ...
            {filteredProjects.length === 0 ? (
              <div className="text-center py-5">
                <i className="fas fa-folder-open text-muted fa-4x mb-3"></i>
                <h3 className="h6">
                  {searchQuery ? "No projects found matching your search" : "No projects"}
                </h3>
                <p className="text-muted mb-4">
        ...
        {activeTab === "active" && (
          <div className="mb-4">
            <div className="d-flex justify-content-between align-items-center mb-3">
              <h2 className="h5 mb-0 text-light">Active Projects</h2>
              <button 
                className="btn btn-success btn-sm"
                onClick={exportToExcel}
              >
            ...
            {filteredProjects.length === 0 ? (
              <div className="text-center py-5">
                <i className="fas fa-tasks text-muted fa-4x mb-3"></i>
                <h3 className="h6">
                  {searchQuery ? "No active projects found matching your search" : "No active projects"}
                </h3>
                <p className="text-muted">
                  {searchQuery ? "Try adjusting your search terms" : "Mark projects as YTS to move them here."}
                </p>
              </div>
            ) : (
              <div className="card">
          ...
          <div className="mb-4">
            ...
            {filteredProjects.length === 0 ? (
              <div className="text-center py-5">
                <i className="fas fa-check-circle text-muted fa-4x mb-3"></i>
                <h3 className="h6">
                  {searchQuery ? "No completed projects found matching your search" : "No completed projects"}
                </h3>
                <p className="text-muted">
                  {searchQuery ? "Try adjusting your search terms" : "Mark active projects as completed to see them here."}
                </p>
              </div>
            ) : (
              <>
