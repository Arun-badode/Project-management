        ...
        <div className="col-12 d-flex justify-content-between align-items-center mb-4">
          ...
          <div className="dropdown">
            ...
            >
              ...
              <span className="badge badge-danger position-absolute top-0 start-100 translate-middle">{activityData.length}</span>
            </button>

            {/* Notification dropdown */}
            {showNotifications && (
              <div className="dropdown-menu dropdown-menu-end p-3" style={{ maxHeight: '400px', overflowY: 'auto' }}>
                    ...
                    {data.tasks.map((task, taskIndex) => (
                      <div key={taskIndex}>
                        ...
                        {task.timestamps.map((timestamp, timestampIndex) => (
                          <small key={timestampIndex} className="d-block text-muted">{timestamp}</small>
                        ))}
                      </div>
                    ))}
                  </div>
        ...
        <div className="card shadow-sm">
          <div className="card-body">
            ...
            {activityData.map((monthData, monthIndex) => (
              <div key={monthIndex} className={`${monthIndex > 0 ? 'mt-4 pt-4 border-top' : ''}`}>
                        ...
                        <td className="text-right">
                          <div>
                            {task.timestamps.map((timestamp, timeIndex) => (
                              <span key={timeIndex} className="d-block text-muted">{timestamp}</span>
                            ))}
                          </div>
                        </td>
                      </tr>
          ...
          <div className="card-body">
            ...
            <div className="list-group">
              ...
              <div className="list-group-item d-flex align-items-start p-3">
                <div className="mr-3 rounded-circle bg-primary text-white d-flex justify-content-center align-items-center" style={{ width: '40px', height: '40px' }}>JD</div>
                <div className="flex-grow-1">
                  <div className="d-flex justify-content-between">
                    <h5 className="font-weight-bold">John Doe</h5>
                    <span className="text-muted small">Today, 11:32 AM</span>
                  </div>
                  <p className="mb-1">Can you share the latest mockups for the Website Redesign project?</p>
                  <span className="badge badge-secondary">Website Redesign</span>
                </div>
              </div>

              {/* Message 2 */}
              <div className="list-group-item d-flex align-items-start p-3">
                <div className="mr-3 rounded-circle bg-success text-white d-flex justify-content-center align-items-center" style={{ width: '40px', height: '40px' }}>AS</div>
                <div className="flex-grow-1">
                  <div className="d-flex justify-content-between">
                    <h5 className="font-weight-bold">Anna Smith</h5>
                    <span className="text-muted small">Yesterday, 3:45 PM</span>
                  </div>
                  <p className="mb-1">I've updated the marketing campaign timeline. Please review when you get a chance.</p>
                  <span className="badge badge-secondary">Marketing Campaign</span>
                </div>
              </div>

              {/* Message 3 */}
              <div className="list-group-item d-flex align-items-start p-3">
                <div className="mr-3 rounded-circle bg-warning text-white d-flex justify-content-center align-items-center" style={{ width: '40px', height: '40px' }}>RJ</div>
                <div className="flex-grow-1">
                  <div className="d-flex justify-content-between">
                    <h5 className="font-weight-bold">Robert Johnson</h5>
                    <span className="text-muted small">Jun 16, 2025</span>
                  </div>
                  <p className="mb-1">The client meeting went well. They approved our proposal for the Q3 deliverables.</p>
